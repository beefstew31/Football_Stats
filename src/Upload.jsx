...
export default function Upload({ season, setLeadersData, onPublished }) {
  ...
  /** Normalize a record from the CSV so that capitalization/aliasing doesn’t matter. */
  function canonical(r) {
    const m = (str) =>
      r[str] ??
      r[str?.toUpperCase?.()] ??
      r[str?.replace?.(/rec_/i, "")];
  ...
  /** Compute schedules, standings, player totals, leaders, logs, and career. */
  function compute(all) {
    const allNorm = all.map(canonical);

    // Season-specific rows
    ...
    Object.values(byGame).forEach((list) => {
      ...
      const resultTuple = (pf, pa) => {
        if (!scored) return "";
        if (pf > pa) return `${pf}-${pa} W`;
        if (pf < pa) return `${pf}-${pa} L`;
        return `${pf}-${pa} T`;
      };

      if (home && away) {
        games.push({
          team: home, date: sample.date, week: sample.week,
          opponent: away, home_away: "H",
          result: resultTuple(hs, as),
        });
        games.push({
          team: away, date: sample.date, week: sample.week,
          opponent: home, home_away: "A",
          result: resultTuple(as, hs),
        });
      } else if (T1 && T2) {
        games.push({ team: T1, date: sample.date, week: sample.week, opponent: T2, home_away: "-", result: "" });
        games.push({ team: T2, date: sample.date, week: sample.week, opponent: T1, home_away: "-", result: "" });
      }
    });
