...
// helper to fetch a JSON file with cache busting
...
export function PlayerPage() {
  ...
  // decode once to get human‑friendly name and team
  ...
  React.useEffect(() => {
    ...
    (async () => {
      setErr('');
      try {
        // use the slug directly, don't encode again
        const logData    = await fetchJson(`stats/${season}/players/logs/${playerSlug}.json`);
        const careerData = await fetchJson(`career/players/${playerSlug}.json`);
        if (live) {
          setLog(Array.isArray(logData) ? logData : []);
          setCareer(Array.isArray(careerData) ? careerData : []);
        }
      } catch (e) {
        ...
        <div className="row" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
          <div className="row" style={{ gap: 12, alignItems: 'center' }}>
            <div style={{
              ...
              background: '#f5f5f5', display: 'grid', placeItems: 'center',
              fontWeight: 700
            }}>
      ...
      <div className="card" style={{ marginTop: 16 }}>
        ...
        {!career.length ? (
          ...
          <table>
            <thead>
              <tr>
                <th>Season</th><th>Team</th><th>Pass Cmp</th><th>Pass Att</th>
                <th>Pass Yds</th><th>TD</th><th>INT</th>
